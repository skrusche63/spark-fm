...
object FMUtil {
  ...
  def read(store:String):(Double,DenseVector,DenseMatrix,Map[String,Any],Seq[FMBlock]) = {
  ...
  def write(store:String,c:Double,v:DenseVector,m:DenseMatrix,p:Map[String,String],blocks:Seq[FMBlock]) {
